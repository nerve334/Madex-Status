...
module.exports.statusPageSocketHandler = (socket) => {
    ...
    socket.on("saveStatusPage", async (slug, config, imgDataUrl, publicGroupList, callback) => {
        try {
            ...
            // Save monitor PIN (null to disable, or exactly 4 digits to enable)
            if (config.monitorPin === null || config.monitorPin === undefined || config.monitorPin === "") {
                statusPage.monitor_pin = null;
            } else {
                const pinStr = String(config.monitorPin).trim();
                if (!/^\d{4}$/.test(pinStr)) {
                    throw new Error("Monitor PIN must be exactly 4 digits (0–9)");
                }
                statusPage.monitor_pin = pinStr;
            }
