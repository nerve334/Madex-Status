...
router.get("/api/status-page/:slug", cache("5 minutes"), async (request, response) => {
    ...
    try {
        ...
        const pin = request.query.pin || null;
        let statusPageData = await StatusPage.getStatusPageData(statusPage, pin);
...
router.get("/api/status-page/heartbeat/:slug", cache("1 minutes"), async (request, response) => {
    ...
    try {
        ...
        // PIN check — if monitor PIN is set, heartbeat data is also gated
        let statusPage = await R.findOne("status_page", " slug = ? ", [slug]);
        if (!statusPage) {
            sendHttpError(response, "Status Page Not Found");
            return;
        }
        if (statusPage.monitor_pin) {
            const pin = request.query.pin || null;
            if (!pin || String(pin) !== String(statusPage.monitor_pin)) {
