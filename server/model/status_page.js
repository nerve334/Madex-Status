...
class StatusPage extends BeanModel {
     ...
     * @param {StatusPage} statusPage Status page to get data for
     * @param {string|null} pin Optional 4-digit PIN to unlock monitored services
     * @returns {object} Status page data
     */
    static async getStatusPageData(statusPage, pin = null) {
        const config = await statusPage.toPublicJSON();

        // Determine if the monitor list should be visible
        const pinEnabled = !!statusPage.monitor_pin;
        const pinUnlocked = !pinEnabled || (pin && String(pin) === String(statusPage.monitor_pin));
        config.monitorPinUnlocked = pinUnlocked;
        ...
        // Public Group List — only populated when PIN is valid (or not set)
    ...
    async toJSON() {
        return {
            ...
            monitorPin: this.monitor_pin || null,
        };
    }
    ...
    async toPublicJSON() {
        return {
            ...
            showOnlyLastHeartbeat: !!this.show_only_last_heartbeat,
            rssTitle: this.rss_title,
            monitorPinEnabled: !!this.monitor_pin,
        };
    }
