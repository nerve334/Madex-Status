    ...
    <div v-if="loadedTheme" class="container mt-3">
        ...
        <div v-if="enableEditMode" class="sidebar" data-testid="edit-sidebar">
            <div class="sidebar-body">
                ...
                <!-- Monitor Access PIN -->
                <div class="my-3">
                    <label for="monitor-pin" class="form-label">
                        <font-awesome-icon icon="lock" class="me-1" style="color: #00d4aa" />
                        Monitor Access PIN
                    </label>
                    <input
                        id="monitor-pin"
                        v-model="config.monitorPin"
                        type="text"
                        class="form-control"
                        maxlength="4"
                        placeholder="4 digits — empty = public"
                    />
                    <div class="form-text">
                        Visitors must enter this 4-digit code to see monitored services. Leave empty to make services
                        publicly visible.
                    </div>
                </div>
        ...
        <div :class="{ edit: enableEditMode }" class="main">
            ...
            <!-- Provider Infrastructure Status (Madex IT Solutions) -->
            <ProviderStatus v-if="!enableEditMode" />
            ...
            <div class="mb-4">
                <!-- PIN Gate — shown when monitor PIN is set and not yet unlocked -->
                <div v-if="config.monitorPinEnabled && !monitorPinUnlocked && !enableEditMode" class="pin-gate shadow-box">
                    <div class="pin-gate-content">
                        <div class="pin-lock-icon">
                            <font-awesome-icon icon="lock" />
                        </div>
                        <h3 class="pin-gate-title">MONITORED SERVICES</h3>
                        <p class="pin-gate-subtitle">Enter the access code to view services</p>

                        <div class="pin-inputs" :class="{ shake: pinShaking }">
                            <input
                                v-for="n in 4"
                                :key="n"
                                :ref="`pinInput${n}`"
                                v-model="monitorPinDigits[n - 1]"
                                type="password"
                                maxlength="1"
                                inputmode="numeric"
                                class="pin-digit"
                                :class="{ error: monitorPinError }"
                                @input="onPinInput(n)"
                                @keydown.backspace="onPinBackspace(n)"
                                @keydown.enter="submitPin"
                            />
                        </div>

                        <p v-if="monitorPinError" class="pin-error-msg">Incorrect code. Please try again.</p>

                        <button
                            class="btn pin-unlock-btn"
                            :disabled="enteredPin.length < 4 || pinLoading"
                            @click="submitPin"
                        >
                            <span v-if="pinLoading" class="spinner-border spinner-border-sm me-2" role="status"></span>
                            <font-awesome-icon v-else icon="unlock" class="me-2" />
                            UNLOCK
                        </button>
                    </div>
                </div>

                <!-- Services list (visible when PIN unlocked or no PIN set) -->
                <template v-else>
                    <div v-if="$root.publicGroupList.length === 0 && loadedData && !config.monitorPinEnabled" class="text-center">
            ...
            <footer class="mt-5 mb-4">
                ...
                <p v-if="config.showPoweredBy" data-testid="powered-by">
                    {{ $t("Powered by") }}
                    <a target="_blank" rel="noopener noreferrer" href="https://madex.at">
                        Madex IT Solutions
                    </a>
                </p>
...
export default {
    ...
    data() {
        return {
            ...
            // Monitor PIN gate
            monitorPinUnlocked: false,
            monitorPinDigits: ["", "", "", ""],
            monitorPinError: false,
            pinLoading: false,
            pinShaking: false,
        };
    },
    computed: {
        ...
        enteredPin() {
            return this.monitorPinDigits.join("");
        },
    ...
    async mounted() {
        ...
        this.getData()
            .then((res) => {
                ...
                // Restore PIN unlock state from sessionStorage or server response
                if (res.data.config.monitorPinUnlocked) {
                    this.monitorPinUnlocked = true;
                } else if (!res.data.config.monitorPinEnabled) {
                    this.monitorPinUnlocked = true;
                }
    ...
    methods: {
         ...
         */
        getData: function () {
            const savedPin = sessionStorage.getItem("statusPagePin_" + this.slug);
            const pinParam = savedPin ? "?pin=" + savedPin : "";

            if (window.preloadData) {
                // If preloadData says PIN is required and we have a saved PIN, bypass preload and fetch fresh
                if (savedPin && window.preloadData.config?.monitorPinEnabled) {
                    return axios.get("/api/status-page/" + this.slug + pinParam);
                }
        ...
        updateHeartbeatList() {
            ...
            if (!this.editMode) {
                const savedPin = sessionStorage.getItem("statusPagePin_" + this.slug);
                const pinParam = savedPin ? "?pin=" + savedPin : "";
                axios.get("/api/status-page/heartbeat/" + this.slug + pinParam).then((res) => {
        ...
        /**
         * Reset logo image to default (public/Asset wh.png)
         * @returns {void}
         */
        resetToDefaultImage() {
            if (!this.editMode) {
            ...
            this.imgDataUrl = "/Asset%20wh.png";
        ...
        /**
         * Handle single PIN digit input — auto-advance focus and auto-submit when full
         * @param {number} n Digit position (1–4)
         * @returns {void}
         */
        onPinInput(n) {
            this.monitorPinError = false;
            const val = this.monitorPinDigits[n - 1];
            // Strip non-digits
            if (val && !/^\d$/.test(val)) {
                this.monitorPinDigits[n - 1] = "";
                return;
            }
            if (val && n < 4) {
                this.$nextTick(() => {
                    const next = this.$refs[`pinInput${n + 1}`];
                    if (next && next[0]) {
                        next[0].focus();
                    }
                });
        ...
        /**
         * Handle backspace in a PIN digit — move focus back when current field is empty
         * @param {number} n Digit position (1–4)
         * @returns {void}
         */
        onPinBackspace(n) {
            if (!this.monitorPinDigits[n - 1] && n > 1) {
                this.$nextTick(() => {
                    const prev = this.$refs[`pinInput${n - 1}`];
                    if (prev && prev[0]) {
                        prev[0].focus();
                    }
                });
        ...
        /**
         * Submit the entered PIN and unlock services if correct
         * @returns {Promise<void>}
         */
        async submitPin() {
            if (this.enteredPin.length < 4 || this.pinLoading) {
                return;
            }
            this.pinLoading = true;

            try {
                const res = await axios.get("/api/status-page/" + this.slug + "?pin=" + this.enteredPin);

                if (res.data.config && res.data.config.monitorPinUnlocked) {
                    sessionStorage.setItem("statusPagePin_" + this.slug, this.enteredPin);
                    this.monitorPinUnlocked = true;
                    this.$root.publicGroupList = res.data.publicGroupList;
                    this.monitorPinError = false;
                    // Also kick off a heartbeat fetch now that we're unlocked
                    this.updateHeartbeatList();
                } else {
                    this.monitorPinError = true;
                    this.pinShaking = true;
                    this.monitorPinDigits = ["", "", "", ""];
                    setTimeout(() => {
                        this.pinShaking = false;
                        this.$nextTick(() => {
                            const first = this.$refs["pinInput1"];
                            if (first && first[0]) {
                                first[0].focus();
                            }
                        });
...
.overall-status {
    ...
    gap: 12px;
    padding: 24px;
    background: linear-gradient(145deg, $dark-card-bg 0%, $dark-bg 100%);
    ...
    .ok {
        color: $primary;
        font-size: 28px;
        filter: drop-shadow(0 0 8px $success-glow);
    }

    .warning {
        color: $warning;
        font-size: 28px;
        filter: drop-shadow(0 0 8px $warning-glow);
    }

    .danger {
        color: $danger;
        font-size: 28px;
        filter: drop-shadow(0 0 8px $danger-glow);
    }
}
...
.logo-wrapper {
    ...
    .icon-upload {
        ...
        border-radius: 8px;
        cursor: pointer;
        box-shadow: 0 4px 12px $success-glow;
        opacity: 0.9;
    }

    /* Reset button placed at top-left of the logo */
    .reset-top-left {
        transition: all $easing-in 0.18s;
        font-size: 14px;
        width: 22px;
        height: 22px;
        padding: 0;
        ...
        &:hover {
            ...
            transform: scale(1.1);
            box-shadow: 0 4px 16px $danger-glow;
            border-color: $danger;
        }
...
.status-maintenance {
    color: $maintenance;
    margin-right: 8px;
    filter: drop-shadow(0 0 8px rgba($maintenance, 0.4));
}
...
.bg-maintenance {
    background: linear-gradient(135deg, $maintenance 0%, darken($maintenance, 10%) 100%) !important;
...
.pin-lock-icon {
    font-size: 40px;
    color: $primary;
    filter: drop-shadow(0 0 14px $success-glow);
    margin-bottom: 4px;
}
...
.pin-unlock-btn {
    ...
    border-radius: 10px;
    color: #0a0d12;
    box-shadow: 0 4px 16px $success-glow;
    transition: all 0.2s ease;

    &:hover:not(:disabled) {
        background: linear-gradient(135deg, $primary-light 0%, $primary 100%);
        box-shadow: 0 6px 24px $success-glow;
...
.pin-inputs.shake {
    animation: shake 0.55s $easing-in-out;
}
</style>
